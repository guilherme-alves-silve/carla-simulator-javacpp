...
public final class CarlaMultiSensorViewer {
    ...
    public static void main(String[] args) throws Exception {
        var cameraLabel = new JLabel("camera: waiting");
        var lidarLabel = new JLabel("lidar: waiting");
        var collisionLabel = new JLabel("collision: none");

        SwingUtilities.invokeLater(() -> {
            var frame = new JFrame("CARLA Multi Sensor Viewer");
        ...
        try (var client = new Client("localhost", 2000)) {
            ...
            try (var world = client.getWorld();
                 ...
                 var vehicle = world.spawnVehicle(blueprints.filter("vehicle.*").get(0), world.getSpawnPoints().get(0));
                 var camera = world.spawnRgbCamera(vehicle, new Transform(new Location(1.6, 0.0, 1.7), new Rotation(0.0, 0.0, 0.0)), 800, 600, 90.0);
                 var lidar = world.spawnLidar(vehicle, new Transform(new Location(0.0, 0.0, 2.5), new Rotation(0.0, 0.0, 0.0)), LidarSensorOptions.defaults());
                 var collision = world.spawnCollisionSensor(vehicle, new Transform(new Location(0.0, 0.0, 0.0), new Rotation(0.0, 0.0, 0.0)))) {
